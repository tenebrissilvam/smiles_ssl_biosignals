...
def compute_swt_features(ecg_signal, wavelet="db4", level=4):
    ...
    coeffs = pywt.swt(
        ecg_signal, wavelet, level=min(level, 4)
    )  # Ограничиваем 4 уровнями
...
def extract_ecg_features(ecg_data, fs=500):
    ...
    rolling_hr = np.full(
        num_samples, compute_rolling_hr(r_peaks, fs)[0] if len(r_peaks) > 1 else 0
    )
...
def augment_ecg_channels(ecg_data, fs=500):
    ...
    for meas_idx in tqdm(range(num_measurements), desc="Обработка замеров"):
        ...
        for feat_idx in range(len(features)):
            augmented_data[meas_idx, feature_offset + feat_idx, :] = np.full(
                signal_length, features[feat_idx]
            )
