...
class ST_MEM_ViT(nn.Module):
    def __init__(
        ...
        # classifier head
        self.head = (
            nn.Identity() if num_classes is None else nn.Linear(width, num_classes)
        )

    def reset_head(self, num_classes: Optional[int] = None):
        del self.head
        self.head = (
            nn.Identity() if num_classes is None else nn.Linear(self.width, num_classes)
        )
    ...
    def forward_encoding(self, series):
        ...
        if num_leads > len(self.lead_embeddings):
            raise ValueError(
                f"Number of leads ({num_leads}) exceeds the number of lead embeddings"
            )
        ...
        left_sep = sep_embedding.expand(b, num_leads, -1, -1) + self.pos_embedding[
            :, :1, :
        ].unsqueeze(1)
        right_sep = sep_embedding.expand(b, num_leads, -1, -1) + self.pos_embedding[
            :, -1:, :
        ].unsqueeze(1)
        x = torch.cat([left_sep, x, right_sep], dim=2)
        lead_embeddings = torch.stack(
            [lead_embedding for lead_embedding in self.lead_embeddings]
        ).unsqueeze(0)
...
def st_mem_vit_small(
    num_leads, num_classes=None, seq_len=2250, patch_size=75, **kwargs
):
    model_args = dict(
        seq_len=seq_len,
        patch_size=patch_size,
