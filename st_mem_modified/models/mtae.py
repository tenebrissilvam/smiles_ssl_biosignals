...
def get_1d_sincos_pos_embed(
    embed_dim: int, grid_size: int, temperature: float = 10000, cls_embed: bool = False
):
...
class MTAE(nn.Module):
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_1d_sincos_pos_embed(
            self.encoder.pos_embedding.shape[-1], self.num_patches, cls_embed=True
        )
        ...
        decoder_pos_embed = get_1d_sincos_pos_embed(
            self.decoder_pos_embed.shape[-1], self.num_patches, cls_embed=True
        )
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        cls_embedding = (
            cls_embedding.expand(b, -1, -1) + self.encoder.pos_embedding[:, :1, :]
        )
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        x_wo_cls = torch.gather(
            x_wo_cls, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, d)
        )
