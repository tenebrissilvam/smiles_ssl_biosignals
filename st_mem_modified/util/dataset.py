...
from util.transforms import get_rand_augment_from_config, get_transforms_from_config
...
class ECGDataset(Dataset):
    ...
    def check_dataset(self):
        ...
        assert all(
            [os.path.exists(fpath) for fpath in fpaths]
        ...
        assert (
            self.target_lead in self._LEAD_SLICE.keys()
...
def build_dataset(cfg: dict, split: str) -> ECGDataset:
    ...
    # Filter out rows with NaN labels if label_col is specified
    if label_col is not None:
        ...
        if original_length != filtered_length:
            print(
                f"Filtered out {original_length - filtered_length} rows with missing labels in {split} split"
            )
    ...
    labels = None
    if label_col is not None:
        # Define consistent label mapping for PTB-XL general diagnosis
        label_mapping = {"CD": 0, "HYP": 1, "MI": 2, "NORM": 3, "STTC": 4}
        labels = df[label_col].map(label_mapping).values
        # Check if there are any unmapped labels (NaN values after mapping)
        if pd.isna(labels).any():
            unmapped_labels = df[label_col][pd.isna(labels)].unique()
            raise ValueError(
                f"Found unmapped labels: {unmapped_labels}. Please update the label_mapping dictionary."
            )
    ...
    label_transform = (
        T.ToTensor(dtype=cfg["label_dtype"]) if labels is not None else None
    )
