...
class Resample:
    ...
    def __init__(
        self, target_length: Optional[int] = None, target_fs: Optional[int] = None
    ) -> None:
...
class SOSFilter:
    ...
    def __init__(
        self, fs: int, cutoff: float, order: int = 5, btype: str = "highpass"
    ) -> None:
...
class _RandomPartialNoise(_Noise):
    ...
    def __init__(
        self, amplitude: float = 0.3, freq: float = 0.5, ratio: float = 0.3
    ) -> None:
...
def get_transforms_from_config(
    ...
    for transform in config:
        if isinstance(transform, str):
            ...
            raise ValueError(
                f"Invalid transform: {transform}, it must be a string or a dictionary."
            )
